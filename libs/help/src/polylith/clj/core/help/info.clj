...
(defn help-text [extended? cm]
  (str "  Shows workspace information.\n"
       ...
       "    " (color/base "bases:" cm) "    1   " (color/lib "libs:" cm) " 4\n"
       "\n"
       ...
       "    -----------------   ---   ----------\n"
       "    " (color/interface "payer" cm) "      " (color/lib "payer" cm) "    " (color/project "s--   st-   --" cm) "\n"
       "    " (color/interface "user" cm) "       " (color/lib "admin" cm) "    " (color/project "s--   ---   s-" cm) "\n"
       "    " (color/interface "user" cm) "       " (color/lib "user" cm) " *   " (color/project "---   st-   --" cm) "\n"
       "    " (color/interface "util" cm) "       " (color/lib "util" cm) "     " (color/project "s--   st-   --" cm) "\n"
       ...
       "     " (color/base "bases:" cm) "    1   " (color/lib "libs:" cm) " 4\n"
       "\n"
       "     Shows how many " (color/project "projects" cm) ", " (color/base "bases" cm) ", " (color/lib "libs" cm) " and " (color/interface "interfaces" cm) " there are\n"
       "     in the workspace.\n"
       "\n"
       ...
       "     -----------------   ---   ----------\n"
       "     " (color/interface "payer" cm) "      " (color/lib "payer" cm) "    " (color/project "s--   st-   --" cm) "\n"
       "     " (color/interface "user" cm) "       " (color/lib "admin" cm) "    " (color/project "s--   ---   st" cm) "\n"
       "     " (color/interface "user" cm) "       " (color/lib "user" cm) " *   " (color/project "---   st-   --" cm) "\n"
       "     " (color/interface "util" cm) "       " (color/lib "util" cm) "     " (color/project "s--   st-   --" cm) "\n"
       "     -          " (color/base "cli" cm) "      " (color/project "s--   st-   --" cm) "\n"
       "\n"
       ...
       "    added to.\n"
       "\n"
       "    The 'interface' column shows what " (color/interface "interface" cm) " the lib has. The name\n"
       "    is the first namespace after the top namespace, e.g.:\n"
       "    " (shared/lib-ns "interface" "user" cm) "\n"
       "\n"
       "    The 'brick' column shows the name of the brick, in " (color/lib "green" cm) " if a lib or\n"
       "    " (color/base "blue" cm) " if a base. Each lib lives in a directory under the 'libs'\n"
       ...
       "    with an asterisk, * (" (color/lib "user" cm) " in this example).\n"
       "\n"
       ...
       "           'libs/user/src' (or is indirectly added by a :local/root).\n"
       "      " (color/project "-t-" cm) "  The project contains a path to the 'test' directory, e.g.\n"
       "           'libs/user/test' (or is indirectly added by a :local/root).\n"
       ...
       "            'libs/user/resources' (or is indirectly added by a :local/root).\n"
       "      " (color/project "--t-" cm) "  The project contains a path to the 'test' directory, e.g.\n"
       "            'libs/user/test' (or is indirectly added by a :local/root).\n"
       "      " (color/project "---x" cm) "  The brick is marked to be executed from this project.\n"
       "\n"
       ...
       "    poly info brick:mylib\n"
       "    poly info brick:mylib:mybase\n"
