...
(defn help-text [cm]
  (str "  Validates the workspace.\n"
       ...
       "    Triggered if a " (s/key ":require" cm) " statement refers to a lib namespace\n"
       "    other than " (color/interface "interface" cm) ". Examples of valid namespaces:\n"
       "     - " (s/lib-ns "interface" cm) "\n"
       "     - " (s/lib-ns "interface.subns" cm) "\n"
       "     - " (s/lib-ns "interface.my.subns" cm) "\n"
       "\n"
       ...
       (color/green cm "defmacro") " definition exists in one lib's\n"
       "    interface but is missing in another lib that uses the same interface.\n"
       "\n"
       ...
       "    Triggered if a base has the same name as a lib or interface.\n"
       "    Projects and profiles can be given any name.\n"
       "\n"
       "  " (color/error cm "Error 106") " - Multiple libs that share the same interfaces in a project.\n"
       "    Triggered if a project contains more than one lib that shares the\n"
       "    same interface.\n"
       "\n"
       "  " (color/error cm "Error 107") " - Missing libs in project.\n"
       "    Triggered if a lib depends on an interface that is not included in the\n"
       "    project. The solution is to add a lib to the project that\n"
       "    implements the interface.\n"
       "\n"
       "  " (color/error cm "Error 108") " - Components with an interface that is implemented by more than one\n"
       "              lib are not allowed for the development project.\n"
       "    The solution is to remove the lib from the development project\n"
       "    and define the deps/paths for each lib in separate profiles\n"
       ...
       "    Triggered if a function or macro is defined in the interface for a lib\n"
       "    but also defined in the same interface for another lib but with a\n"
       ...
       "\n"
       "  " (color/warning cm "Warning 207") " - Unnecessary libs were found in project.\n"
       "    Triggered if libs were defined in a project that are not used by any of\n"
       "    its bricks. Development is only checked if " (s/key ":dev" cm) " is passed in and is only performed\n"
       "    by the check command (not test and info). To ignore this warning, put the lib\n"
