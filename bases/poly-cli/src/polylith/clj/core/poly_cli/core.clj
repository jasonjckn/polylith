...
(defn -main
  "This is the entry point of the 'poly' command.
   ...
   It reads the input from the incoming arguments and delegates to the 'command' lib,
   ...
   - The last step change/with-changes, operates on the enhanced hash map, which again is an
     in-memory representation of the workspace. It calculates what libs, bases and projects
...
(comment
  ...
  (-main "ws" "get:libs:account:namespaces:src:core:file-path" "ws-file:../sandbox/furkan.edn" ":no-exit")
  (-main "ws" "get:libs:api:namespaces:src:core:file-path" "ws-dir:../../Downloads/polylith-0.1.0-alpha9/" ":no-exit")
  (-main "check" ":no-exit")
  ...
  (-main "ws" "get:libs:without-src:non-top-namespaces" "ws-dir:examples/local-dep" ":no-exit")
