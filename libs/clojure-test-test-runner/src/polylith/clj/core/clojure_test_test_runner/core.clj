...
(defn libs-msg [lib-names color-mode]
  (when (seq lib-names)
...
(defn run-message [project-name libs bases bricks-to-test projects-to-test color-mode]
  (let [lib-names (into #{} (map :name) libs)
        ...
        libs-to-test (filterv #(contains? lib-names %) bricks-to-test)
        libs-to-test-msg (libs-msg libs-to-test color-mode)
        projects-to-test-msg (when (seq projects-to-test)
                               [(color/project (str/join ", " projects-to-test) color-mode)])
        entities-msg (str/join ", " (into [] cat [libs-to-test-msg
                                                  bases-to-test-msg
...
(defn create
  ...
  (let [{:keys [bases libs]} workspace
        ...
        test-statements* (->> [(brick-test-namespaces (into libs bases) @bricks-to-test*)
    ...
    (reify test-runner-contract/TestRunner
      ...
      (run-tests [this {:keys [color-mode eval-in-project is-verbose] :as opts}]
        (when (test-runner-contract/tests-present? this opts)
          (let [run-message (run-message name libs bases @bricks-to-test*
