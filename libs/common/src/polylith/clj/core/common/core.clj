...
(defn absolute-path
  "entity-root-path will be passed in as e.g. 'libs/invoicer' if a brick,
...
(defn find-brick [name {:keys [libs bases]}]
  (let [bricks (concat libs bases)]
    (util/find-first #(= name (:name %)) bricks)))

(defn find-lib [name libs]
  (util/find-first #(= name (:name %)) libs))
