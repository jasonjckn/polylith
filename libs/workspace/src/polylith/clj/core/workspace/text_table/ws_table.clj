...
(defn lib-sorter [{:keys [interface name]}]
  [(:name interface) name])

(defn table [{:keys [settings projects libs bases paths changes]} is-show-loc is-show-resources]
  (let [{:keys [color-mode thousand-separator]} settings
        ...
        sorted-libs (sort-by lib-sorter libs)
        bricks (concat sorted-libs bases)
        ...
        ifc-column (ifc-column/column sorted-libs bases)
