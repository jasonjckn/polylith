...
(defn bricks-to-test-for-project [{:keys [is-dev alias name base-names lib-names]}
                                  source
                                  settings
                                  changed-projects
  ...
  (let [include-project? (or (or (contains? selected-projects name)
        ...
        bricks-for-source (if (= :test source)
                            ...
                            (into #{} (mapcat :test) [base-names lib-names])
                            ...
                            ;; in base-names/lib-names (not :src).
                            (set/intersection (into #{} (mapcat :src) [base-names lib-names])
                                              (into #{} (mapcat :test) [base-names lib-names])))
