...
(defn brick-ids-in-project [lib-names base-names bricks]
  (set (concat (map #(-> % :interface :name)
                    (filter #(contains? (set lib-names) (:name %))
                            bricks))
               (map :name
...
(defn finalize-deps
  "At this moment, all the dependencies for a specific project has been calculated and are stored
   ...
   During those calculations, we also make sure we translate interfaces names to lib names
   (used by the project)."
...
(defn brick-names-to-ids
  "Converts lib names to interface names so that they can be used in the dependency calculations.
...
(defn project-deps
  [libs bases lib-names-src lib-names-test base-names-src base-names-test suffixed-top-ns brick-names-to-test]
  "Calculate the src and test dependencies for a project. The returned dependencies
   ...
   A project can only have one lib per interface (one implementation) and we take advantage of this by
   translating all lib names to their interface names when calculating the dependencies for a project.
   ...
   not available). A brick's test context often only depends on other lib's src context, but it's also allowed
   ...
   A project can have a different set of libs for src and test (e.g. a test-helper only used in test)
  ...
  (let [brick-names (set (concat lib-names-src lib-names-test base-names-src base-names-test))
        bricks (filter #(contains? brick-names (:name %))
                       (concat bases libs))
        lib-names (set (concat lib-names-src lib-names-test))
        ;; Make sure we pick the right lib if more than one for an interface.
        ifc->comp (into {} (map (juxt #(-> % :interface :name) :name)
                                (concat libs
                                        (filter #(contains? lib-names (:name %))
                                                libs))))
        brick-ids-to-test (brick-names-to-ids brick-names-to-test libs)
        all-brick-ids (set (concat (map #(-> % :interface :name) libs)
                                   (map :name bases)))
        brick-ids-in-project-src (brick-ids-in-project lib-names-src base-names-src bricks)
        brick-ids-in-project-test (brick-ids-in-project (concat lib-names-src lib-names-test) (concat base-names-src base-names-test) bricks)
        test-only-brick-ids (set/difference brick-ids-in-project-test brick-ids-in-project-src)
        all-bricks (concat libs bases)
        ...
        ;; Here we store all our dependencies in a map. Each key is a brick-id (a lib interface name or base name) and
    ...
    ;; Step 2: For each brick, convert interface names to lib names + calculate missing and circular dependencies.
