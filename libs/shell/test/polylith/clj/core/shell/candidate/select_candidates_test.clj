...
(deftest create-lib
  (is (= ["interface" "name"]
         (candidates "create" :next "lib"))))

(deftest create-lib-
  (is (= ["name"]
         (candidates "create" :next "lib" :next ""))))

(deftest create-lib-name
  (is (= []
         (candidates "create" :next "lib" :next "name" ""))))

(deftest create-lib-name-c1
  (is (= []
         (candidates "create" :next "lib" :next "name" "c1"))))

(deftest create-lib-name-c1-
  (is (= ["interface"]
         (candidates "create" :next "lib" :next "name" "c1" :next ""))))

(deftest create-lib-name-c1-interface
  (is (= [""]
         (candidates "create" :next "lib" :next "name" "c1" :next "interface"))))

(deftest create-lib-name-c1-interface-i1
  (is (= []
         (candidates "create" :next "lib" :next "name" "c1" :next "interface" "i1"))))
...
(deftest help-create-lib
  (is (= []
         (candidates "help" :next "create" :next "lib"))))
...
(deftest ws-get
  (is (= ["bases" "changes" "libs" "interfaces" "messages" "name" "paths"
...
(deftest ws-get-libs-shell-interface
  (is (= ["interface" "interface-deps"]
         (candidates "ws" :next "get" "libs" "shell" "interface"))))

(deftest ws-get-libs-shell-interface-
  (is (= ["definitions" "name"]
         (candidates "ws" :next "get" "libs" "shell" "interface" ""))))

(deftest ws-get-libs-shell-interface-definitions-
  (is (= ["start"]
         (candidates "ws" :next "get" "libs" "shell" "interface" "definitions" ""))))

(deftest ws-get-libs-api-non-top-namespaces-
  (is (= []
         (setup/candidates "ws" :next "get" "libs" "api" "non-top-namespaces" ""))))

(deftest ws-out-libs-next-
  (is (= [":all" ":all-bricks" ":dev" ":latest-sha" ":loc" ":project"
          "brick" "get" "project" "since"]
         (candidates "ws" :next "out" "libs" :next ""))))
