...
(defn src-lib-deps-from-bricks
  ...
  (let [src-brick-names (brick-deps/extract-brick-names is-dev project-src-deps)
        ...
        project-lib-deps (bil/with-sizes-vec
                           ws-dir
                           entity-root-path
        ...
        lib-deps (-> project-lib-deps
                     ...
                     (bil/resolve-libs override-src-deps))]
    [lib-deps project-lib-deps]))
...
(defn test-lib-deps-from-bricks
  ...
  (let [all-brick-names (brick-deps/extract-brick-names is-dev (concat project-src-deps project-test-deps))
        ...
        lib-deps (-> project-lib-deps
                     ...
                     (bil/resolve-libs (merge override-src-deps override-test-deps))
                     set
                     sort
                     vec)]
    [lib-deps project-lib-deps]))
...
(defn read-project
  ([{:keys [deps project-name project-dir project-config-dir is-dev]} ws-dir name->brick project->settings user-home suffixed-top-ns interface-ns]
   (let [{:keys [paths deps override-deps aliases mvn/repos]} deps
         ...
         entity-root-path (when (not is-dev) (str "projects/" project-name))
         override-src-deps (bil/latest-with-sizes ws-dir entity-root-path (if is-dev (-> aliases :dev :override-deps) override-deps) user-home)
         override-test-deps (bil/latest-with-sizes ws-dir entity-root-path (-> aliases :test :override-deps) user-home)
