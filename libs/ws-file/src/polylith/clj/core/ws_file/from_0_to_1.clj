...
(defn convert-brick [{:keys [lib-dep-names
                             ...
                             lines-of-code-test
                             namespaces-src
                             namespaces-test] :as lib}]
  (assoc (dissoc lib :lib-dep-names
                           :lib-imports-src
                           :lib-imports-test
                           :lines-of-code-src
                           :lines-of-code-test
...
(defn convert
  ...
  [{:keys [settings bases libs projects changes] :as ws}]
  (assoc ws :settings (convert-settings settings)
            :bases (mapv convert-brick bases)
            :libs (mapv convert-brick libs)
