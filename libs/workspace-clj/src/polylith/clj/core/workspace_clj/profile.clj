...
(defn brick-name [[_ {:keys [local/root]}]]
  (when root
    (cond
      ...
      (str/starts-with? root "libs/") (str-util/skip-prefix root "libs/"))))
...
(defn profile [ws-dir [profile-key {:keys [extra-paths extra-deps]}] name->brick user-home]
  (let [;; :extra-paths
        ...
        path-lib-names (vec (sort (select/names path-entries c/lib?)))
        project-names (vec (sort (select/names path-entries c/project?)))
        ;; extra-deps
        deps-base-names (mapcat #(brick-name % "bases/") extra-deps)
        deps-lib-names (mapcat #(brick-name % "libs/") extra-deps)
        deps-brick-names (concat deps-base-names deps-lib-names)
        ...
        lib-names (vec (sort (set (concat path-lib-names deps-lib-names))))
        paths (vec (sort (set (concat extra-paths deps-brick-paths))))
        lib-deps (bil/latest-with-sizes ws-dir nil
                                        (concat brick-libs
