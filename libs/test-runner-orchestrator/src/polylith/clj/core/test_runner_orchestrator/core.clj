...
(defn print-bricks-to-test [lib-names base-names bricks-to-test color-mode]
  (when bricks-to-test
    (let [libs-to-test (sort (set/intersection lib-names (set bricks-to-test)))
          bases-to-test (sort (set/intersection base-names (set bricks-to-test)))
          libs (libs-msg libs-to-test color-mode)
          bases (bases-msg bases-to-test color-mode)
          bricks (str/join ", " (concat libs bases))]
      (println (str "Bricks to run tests for: " bricks)))))

(defn run [{:keys [libs bases projects changes settings messages] :as workspace} is-verbose color-mode]
    ...
    (let [start-time (time-util/current-time)
      ...
      (if (empty? projects-to-test)
        ...
        (do
          ...
          (print-bricks-to-test lib-names base-names bricks-to-test color-mode)
          (println)
