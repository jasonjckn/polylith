...
(comment
  (def workspace (-> "libs/test-helper/resources/workspace.edn"
                     slurp read-string))
...
(comment
  (reset! ws nil)
  (reset! ws (-> "libs/test-helper/resources/workspace.edn"
                 slurp read-string))
  ...
  (select-candidate ["ws" :next "get" "libs" "shell" "interface" ""])
  (select-candidate ["ws" :next "get" "libs" "shell" "interface"])
  (select-candidate ["ws" :next "get" "libs" "shell" ""])

  (with-potential-exact-match ["deps" :next "brick" "deployer" :next "project" ""])


  (select-candidate ["ws" :next "out" "libs" :next ""])
